...
import { ensureParentFolderExists } from './ensureFolderExists';
...
export class PathProposer {
    ...
    private static getConfigKey(key: string): unknown | string {
        let value: unknown | string = vscode.workspace.getConfiguration(SECTION_CONF).get(key);
        return value;
    }
    ...
    public static replaceVariables(pattern: string, context: ContextValues = {}): string {
    ...
    private static getDefaultExportedObjectsFolder(): string {
        ...
        if ("devsecops" === this.getConfigKey(MODE)) {
            return path.join(
                workplaceFolder,
                'iiq-xml-config/src/main/config'
            )
        }
    ...
    public static getExportedObjectsFolder(
        environment: string
    ): string {
    ...
    private static getDefaultExportedObjectFilename(
        ...
        const tempFile = tmp.tmpNameSync({ prefix: `${objectType}-${objectName}`, postfix: '.xml' });
        return tempFile
    }
